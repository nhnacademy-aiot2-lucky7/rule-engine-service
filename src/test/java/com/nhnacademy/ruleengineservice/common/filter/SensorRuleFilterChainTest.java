...
class SensorRuleFilterChainTest {
    ...
    void testFilter_passAllRules() {
        ...
        when(sensorRuleService.getRulesByKey(gatewayId, deviceId, dataType)).thenReturn(Arrays.asList(rule));
        ...
        List<Rule> result = sensorRuleViolationChecker.getViolatedRules(sensorData);

        // Then: 필터가 통과해야 함
        assert(result);

        // verify()로 호출 여부 확인
        verify(sensorRuleService, times(1)).getRulesByKey(gatewayId, deviceId, dataType);
    ...
    void testFilter_failsAtFirstRule() {
        ...
        // Mock behavior: getRulesByKey 호출 시 deviceId와 dataType 인자를 정확하게 설정
        when(sensorRuleService.getRulesByKey(gatewayId, deviceId, dataType)).thenReturn(Arrays.asList(rule)); // 룰 리스트 반환
        ...
        // Then
        assert(!result); // 룰을 하나라도 통과하지 못하면 false 반환

        // verify()로 호출 여부 확인
        verify(sensorRuleService, times(1)).getRulesByKey(gatewayId, deviceId, dataType); // 올바른 인자 사용 확인
        verify(rule, times(1)).getValue();
        verify(rule, times(1)).getOperator();
    }
}